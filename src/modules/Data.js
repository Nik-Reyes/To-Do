...
export default class Data {
	...
	deleteCheckedTask(taskToDelete) {
		const idx = this.completedList.tasks.findIndex(task => task.id === taskToDelete.id);
		if (idx !== -1) {
	...
	deleteTaskFromScheduled(taskToDelete) {
		...
		const idx = scheduledListTasks.findIndex(task => task.id === taskToDelete.id);
		if (idx !== -1) {
	...
	handleNewDate(taskElementIdx, newDate) {
		...
		if (scheduledHasTask || todayHasTask) {
			...
			} else if (isAfter(selectedDate, todaysDate) && isAfter(selectedDate, prevDate)) {
				this.deleteTaskFromToday(dateTask);
	...
	init() {
		...
		this.#destinationService['All Tasks'].tasks = this.getFlattenedSortedMyLists();
