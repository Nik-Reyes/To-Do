...
export default class RenderUI {
	...
	#taskCollections = {
		'generic': collectionState => {
			this.taskCollectionWrapper.querySelector('nav').replaceWith(createGenericNav());
			return createGenericTaskCollection(collectionState);
		},
		'All Tasks': collectionState => {
			this.taskCollectionWrapper.querySelector('nav').replaceWith(createDisabledNav());
	...
	createListElements(sysObjects, myObjects, attributes) {
		let i = 0;
		const sysEls = sysObjects.map(list => createSystemListElement(list, attributes?.[i++]));
		const myEls = myObjects.map(list => createMyListElement(list, attributes?.[i++]));
	...
	renderSections(sections, classes, sectionStates) {
		...
		sections.forEach((section, i) => {
			...
			const buttonText = classes[i].class.includes('sub-collection-expanded') ? 'Collapse' : 'Expand';

			const sectionHeader = this.renderSectionHeader(section.listTitle, buttonText, sectionStates[i], section.id);
	...
	async renderSectionedTasks(sections, classes, sectionHasTasks) {
		...
		const sectionsFragment = this.renderSections(sections, classes, sectionHasTasks);
	...
	renderLists(sysObjects, myObjects) {
		...
		const [permaLists, userLists] = this.createListElements(sysObjects, myObjects);
		if (!permaLists) throw Error('Perma Lists Elements DNE!');
		if (!userLists) throw Error('User Lists Elements DNE!');

		this.sidebar.querySelector('.inner-system-list-wrapper').append(...permaLists);
	...
	rerenderLists(sysObjects, myObjects, systemListWrapper, mylistWrapper, classes) {
		const [permaLists, userLists] = this.createListElements(sysObjects, myObjects, classes);
	...
	setTaskCollectionElements(taskCollectionType, collectionState) {
		const newTaskCollection = this.taskCollections[taskCollectionType](collectionState);
	...
	renderAllTasksCollection(sectionedTasks, classes, sectionHasTasks, collectionState) {
	...
	renderTaskCollectionWrapper() {
		...
		this.taskCollection = this.taskCollectionWrapper.querySelector('.task-collection');
	}
	...
	init(title, systemLists, myLists, sectionedTasks, classes, sectionHasTasks, pageWrapper, collectionState) {
		this.createHeaderElement(title);
		this.createSidebarElement();
		this.renderLists(systemLists, myLists);
		this.renderTaskCollectionWrapper();
		this.renderAllTasksCollection(sectionedTasks, classes, sectionHasTasks, collectionState);
