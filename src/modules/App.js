...
export default class App {
	...
	getSectionHeader(id) {
		const sectionWrappers = Array.from(this.elements.taskCollectionWrapper.querySelectorAll('.section-wrapper'));
	...
	getAllListElementBtnClasses() {
		return Array.from(this.elements.sidebar.querySelectorAll('.list-btn')).map(btn => {
	...
	getTaskIdxFromElement(taskWrapper) {
		return [...this.elements.taskCollectionWrapper.querySelectorAll('.task-wrapper')].indexOf(taskWrapper);
	}
	...
	getSectionElements() {
		return Array.from(this.elements.taskCollectionWrapper.querySelectorAll('.sub-collection-wrapper'));
	}
	...
	replaceNewList(editableListElement, target) {
		...
		this.renderer.updateHeaderTitle(this.elements.headerTitle, this.data.currentListTitle);
		this.renderer.renderGenericTaskCollection(this.data.currentTasks, this.data.getTaskCollectionState());
	}
	...
	requestDisabledTasksCollection() {
		this.renderer.renderDisabledCollection(this.data.currentTasks, this.data.getTaskCollectionState());
	}
	...
	confirmListEdit(listElementIdx, listBtnWrapper, input) {
		...
		if (this.data.currentListIsAllTasks()) {
			...
			this.renderer.rerenderSectionHeader(sectionHeader, list.title, null, listHasTasks, list.id);
		}
	...
	openEditOptions(listBtnWrapper) {
		const hoverWrapper = listBtnWrapper.querySelector('.hoverable-list-content-wrapper');
	...
	handleSidebarClicks(e) {
		...
		if (e.target.closest('.addList-btn')) {
			...
			const listElementHandlers = {
				...
				'.confirm-edit-btn': () => this.confirmListEdit(listElementIdx, listBtnWrapper, listInput),
				'.cancel-edit-btn': () => this.cancelListEdit(listElementIdx, listBtnWrapper),
			};
	...
	handleDocumentClicks(e) {
		...
		const opendMenu = this.elements.taskCollectionWrapper.querySelector('.open-menu');
		const focusedDate = this.elements.taskCollectionWrapper.querySelector('.task-date.focused');
		const focusedPriorityWrapper = this.elements.taskCollectionWrapper.querySelector('.priority-btn-wrapper.focused');
		const currentListEdit = document.querySelector('.list-btn-wrapper:has(.expand-row)');
		const textAreas = this.getTextareas();

		if (textAreas.length && !e.target.closest('.add-task-btn')) {
			if (!target.className.includes('task-input') && !target.className.includes('task-notes')) {
		...
		if (newList && !target.closest('.new-list') && !target.closest('.addList-btn')) {
		...
		if (opendMenu && !target.closest('.task-priority') && !target.closest('.open-menu')) {
			opendMenu.classList.remove('open-menu');
		}

		if (currentListEdit) {
			if (!e.composedPath().includes(currentListEdit)) {
				currentListEdit.querySelector('.hoverable-list-content-wrapper').classList.remove('expand-row');
			}
		}
	}
	...
	handlePriorityOptionClick(target, taskWrapper, taskIdx) {
		const menu = this.elements.taskCollectionWrapper.querySelector('.open-menu');
		this.closeMenu(menu);
	...
	addTaskElement(e) {
		...
		const taskCollection = this.elements.taskCollectionWrapper.querySelector('.task-collection');
	...
	handleTaskInput(e) {
		...
		const property = Object.keys(propMap).find(className => target.className.includes(className));
		...
		if (property) {
			if (property === 'task-checkbox') {
				...
				this.data.updateTaskObjectWithIndex(taskIdx, propMap[property], target.value);
	...
	taskResize() {
		const textarea = this.elements.taskCollectionWrapper.querySelector('textarea');
		if (!textarea) return;
	...
	applyEventListeners() {
		...
		this.elements.sidebar.addEventListener('click', this.handleSidebarClicks.bind(this));
		this.elements.sidebar.addEventListener('keydown', this.handleSidebarInput.bind(this));
		this.elements.mylistWrapper.addEventListener('keydown', this.handleNewList.bind(this));
		document.addEventListener('click', this.handleDocumentClicks.bind(this));
		this.elements.taskCollectionWrapper.addEventListener('click', this.handleTaskWrapperClicks.bind(this));
		this.elements.taskCollectionWrapper.addEventListener('input', this.handleTaskInput.bind(this));
		this.elements.taskCollectionWrapper.addEventListener('keydown', this.handleTaskKeydown.bind(this));
		this.elements.taskCollectionWrapper.addEventListener('dblclick', this.handleDoubleClicks.bind(this));
	}
	...
	deleteTaskElement(taskWrapper, taskIdx) {
		...
		if (this.data.currentListIsAllTasks()) {
			...
			if (!listHasTasks) {
				const sectionHeader = this.getSectionHeader(list.id);
				if (!sectionHeader) return;
				this.renderer.rerenderSectionHeader(sectionHeader, list.title, null, listHasTasks, sectionHeader.dataset.id);
			}
			return;
		}
	...
	handleTaskWrapperClicks(e) {
		...
		if (target.closest('.add-task-btn')) {
			...
			if (subCollectionWrapper) {
				...
				target.innerText = target.innerText.toLowerCase() === comparitor ? 'expand' : comparitor;
				this.setSectionStates();
			}
			...
			const taskClickHandlers = {
				...
				'priority-menu-option': () => this.handlePriorityOptionClick(target, taskWrapper, taskIdx),
			};
	...
	toggleButton(button) {
		button.disabled ? button.removeAttribute('disabled') : button.setAttribute('disabled', '');
	}
	...
	removeActiveTask() {
		const activeTask = this.elements.taskCollectionWrapper.querySelector('.active');
	...
	handleTaskKeydown(e) {
		...
		if (key === 'Escape') {
			if (target.closest('.task-notes') || target.closest('.task-input')) {
				...
				const taskWrapper = target.closest('.task-wrapper');
				const menu = taskWrapper.querySelector('.priority-menu-wrapper.open-menu');
				if (menu) {
